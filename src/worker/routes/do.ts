...
export async function handleDOGenerationLogs(request: Request, env: Env): Promise<Response> {
  const v = await verifyAdmin(request, env);
  if (!v.ok) return json({ error: v.error }, 401);
  ...
  try {
    ...
    const doUrl = new URL("http://localhost/generation-logs");
    url.searchParams.forEach((val, key) => doUrl.searchParams.set(key, val));
    ...
    console.error("[DO Proxy] generation-logs error:", e);
    return json({ error: e.message }, 500);
  }
}
