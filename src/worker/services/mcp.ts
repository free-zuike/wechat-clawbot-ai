...
export async function callAIWithMCP(
  ...
  for (const tool of tools) {
    ...
    if (server) {
      ...
      if (!server.tools.find(t => t.name === tool.name)) server.tools.push(tool);
    }
  }
  ...
  for (let round = 0; round < maxRounds; round++) {
    const resp = await fetch(apiUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey_}` },
      body: JSON.stringify({ model: model || "gpt-4o", messages, max_tokens: maxTokens, temperature, tools: openAITools }),
    });
    ...
    if (!message) throw new Error("MCP AI 响应格式异常");

    messages.push({ role: "assistant", content: message.content || "", ...(message.tool_calls ? { tool_calls: message.tool_calls } : {}) } as any);

    const toolCalls = message.tool_calls;
    if (!toolCalls || toolCalls.length === 0) return message.content || "";

    const parsedCalls = parseToolCalls(toolCalls, tools);
    const results = await executeToolCalls(parsedCalls, db);

    for (const result of results) {
      messages.push({ role: "tool", tool_call_id: result.callId, content: result.content } as any);
    }
